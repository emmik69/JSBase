 ...
 * @returns {{ fname: string, sname: string: , lname: string, bdate, startlearn: int, fac: string}} Объект дела
 */
function makeTodoItemFromData(data) {
  ...
  const todoItem = {
    fname: data.fname && String(data.fname),
    sname: data.sname && String(data.owner),
    lname: data.lname && String(data.owner),
    bdate,
    startlearn: data.startlearn && int(data.startlearn),
    fac: data.fac && String(data.fac),
  };

  // проверяем, все ли данные корректные и заполняем объект ошибок, которые нужно отдать клиенту
  if (!todoItem.fname)
    errors.push({ field: 'fname', message: 'Не указана фамилия' });
  if (!todoItem.sname)
    errors.push({ field: 'sname', message: 'Не указано имя' });
  if (!todoItem.lname)
    errors.push({ field: 'lname', message: 'Не указано отчество' });
  if (!todoItem.bdate)
    errors.push({ field: 'bdate', message: 'Не указана дата' });
  if (!todoItem.startlearn)
    errors.push({
      field: 'startlearn',
      message: 'Не указана дата начала обучения',
    });
  if (!todoItem.fac)
    errors.push({
      field: 'fac',
      message: 'Не указан факультет',
    });
 ...
 * @param {{ fname: string }} [params] - Фильтр по владельцу дела
 * @returns {{ fname: string, sname: string: , lname: string, bdate, startlearn: int, fac: string}[]} Массив дел
 */
